...
def _generate_widevine_data(
  key_ids,
  content_id,
  provider,
  protection_scheme,
  force_protection_scheme: bool = False,
):
  ...
  # 'cenc' is the default, so omitted by default to save bytes.
  if protection_scheme:
    if force_protection_scheme or protection_scheme != 'cenc':
...
def main(
    ...
    if ns.system_id == WIDEVINE_SYSTEM_ID:
      ...
      if not pssh_data:
        ...
        pssh_data = _generate_widevine_data(
          ns.key_id, ns.content_id,
          ns.provider, ns.protection_scheme,
          force_protection_scheme=force_protection_scheme,
        )
    else:
  ...
  if output_format == 'human' or not output_format:
    for box in boxes:
      return box.human_string()
  else:
    box_data = b''.join([x.binary_string() for x in boxes])
    if output_format == 'hex':
      return base64.b16encode(box_data).decode()
    else:
      return base64.b64encode(box_data).decode()
